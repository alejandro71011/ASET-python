...
class Credito:
    ...
    def pagoMensual(self):
        if self.__rsolicitud=="A":
            pasoA = (float(self.getTipoCredito().getInteresAnual()) / 100) / 12
            pasoB = pasoA + 1
            pasoC = pasoB**(self.getPlazodepago() * -1)
            pasoD = float(1 - pasoC)
            pasoE = float(pasoA * self.getDinerosolicitado())
            return float(pasoE / pasoD)
        else:
...
ppinversion=TipoCredito(1,19,10000000,84,"Prestamo Personal Inversion")#15.75,10000000,84,"Prestamo Personal Inversion")
...
credito1=Credito(3000000, "12/06/2015", "12/06/2014", 0, 42, asociado1, ppinversion)
